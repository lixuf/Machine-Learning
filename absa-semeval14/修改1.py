...
undic=10000
for i in total_data.keys():
    for i2 in range(len(total_data[i]["text"])):
        if total_data[i]["text"][i2]  in word_index:
            total_data[i]["text"][i2]=word_index[total_data[i]["text"][i2]]
        else:
            word_index[total_data[i]["text"][i2]]=undic
            total_data[i]["text"][i2]=word_index[total_data[i]["text"][i2]]
            undic+=1
def word2vec(in_txt):
    print(in_txt[0])
    (x_train_in1, y_train1), (x_test_in2, y_test2) = reuters.load_data()

    in_txt=list(in_txt)+list(x_train_in1)
    in_txt=in_txt+list(x_test_in2)
    print(in_txt[10])
    print(len(in_txt))
    for i in range(len(in_txt)):
        for i2 in range(len(in_txt[i])):
            in_txt[i][i2]=str(in_txt[i][i2])
    path = get_tmpfile("word2vec_lstm.model")
    model = Word2Vec(in_txt, size=300, window=6, min_count=0,
...
for i in total_data.keys():
    senten=[]
    avag+=len(total_data[i]["text"])
    for i2 in range(0,len(total_data[i]["text"])):
        vector = model.wv[str(total_data[i]["text"][i2])]
        senten.append(vector)
    total_data[i]["text"]=senten
avag=avag/len(total_data)
print('avag',avag)
...
for i in total_data.keys():
    if len(total_data[i]["text"])>=maxlen:
        total_data[i]["text"]=total_data[i]["text"][:maxlen]
    else:
        total_data[i]["text"]=seq_padding( total_data[i]["text"],np.zeros(shape=(word_size,)))
for i in total_data_un.keys():
    if len(total_data_un[i]["text"])>=maxlen:
        total_data_un[i]["text"]=total_data_un[i]["text"][:maxlen]
    else:
...
ps1 = Lambda(lambda x: x[0] * x[1])([ps1, mask])
ps2 = Lambda(lambda x: x[0] * x[1])([ps2, mask])


fix_model=Model([t1_in,t2_in],t1)
...
def extract_items(text_in,text_in_un,aspect):
    ...
    print("aspect",aspect)
    print("fix",fix_model.predict([text_in_un, text_in]))
