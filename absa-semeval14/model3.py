...
for i in range(103):
    list_index.append(i)
list_index=K.one_hot(list_index,102)
...
for i in total_data.keys():
    ...
    while len(preindex)<maxlen:
        preindex.append(list_index[1])
        postindex.append(list_index[1])
    for i1 in range(len(total_data[i]["aspect"])):
        ...
        preindex[total_data[i]['aspect'][i1][0]]=list_index[0]
        total_data[i]['aspect'][i1][1]=total_data[i]['text'].index(total_data[i]['aspect'][i1][3][-1]) 
        postindex[total_data[i]['aspect'][i1][1]]=list_index[0]
        sum_1=sum_1+1
        sum_0=sum_0-1
    for i1 in range(len(preindex)):
        if preindex[i1]==list_index[1]:
            preindex[i1]=list_index[tag_0]
            tag_0=tag_0+1
            if tag_0>=20:
                tag_0=1
    for i1 in range(len(postindex)):
        if postindex[i1]==list_index[1]:
            postindex[i1]=list_index[tag_0]
            tag_0=tag_0+1
            if tag_0>=20:
                tag_0=1
    for i1 in range(len(total_data[i]['text'])-1,maxlen):
        sum_2=sum_2+1
        preindex[i1]=list_index[tag_2]
        postindex[i1]=list_index[tag_2]
        tag_2=tag_2+1
        if tag_2>=102:
            tag_2=20
...
print("list_index[0]",list_index[0])
...
s1_in = Input(shape=(None,102))
s2_in = Input(shape=(None,102))
...
pn1 = Dense(102, activation='softmax')(pn1)
pn2 = Dense(word_size, activation='relu')(t)
pn2 = Lambda(mask_sigmoid)([pn2, mask])
pn2 = Dense(102, activation='softmax')(pn2)
...
def loss(y_true, y_pred):
    alpha=0.5
    yita=2
    theta = lambda t: K.pow(t,yita)
    return -  (y_true * K.log(y_pred + 1e-8)*alpha*theta(1-y_pred) + (1 - y_true) * K.log(1 - y_pred + 1e-8)*(1-alpha)*K.pow(y_pred,yita))
    ...
    def __iter__(self):
        while True:
            ...
            for i in idxs:
                ...
                T1.append(text_un)
                T2.append(text)
                s1=self.data[0][i]["index"][0]
                s2=self.data[0][i]["index"][1]
                S1.append(s1)
                S2.append(s2) 
