...
def model2():#模型主体2
	...
	model=Model(input=[sen_in,mask_in,position_in],outputs=result)
	model.summary()
	return model,crf,model_part_b2,model_part_a

model,crf,model_part_b2,model_part_a=model2()
...
class Evaluator(Callback):
	...
	def on_epoch_end(self,epoch,logs=None):
		...
		if f1>=self.best_val_f1:
			self.best_val_f1=f1
			model_part_b2.save_weights('best_model_model_part_b2.weights')
		print(
...
#adam=l_bei(Adam)
#adam=adam(paramwise_lr_schedule={"gru0":0.01,"gru1":0.01,"gru2":0.01,"cap":0.1})
#adam=l_shiying(Adam)
#adam=adam()
#adam=l_fenduan(Adam)
